  ...
  try {
    ...
    return res.status(error.response?.status || 500).json({
      ...
      message:
        ...
        'Не вдалося отримати поточну погоду',
    });
  }
};
...
export const getForecast = async (req, res) => {
  try {
    ...
    return res.status(error.response?.status || 500).json({
      ...
      message:
        error.response?.data?.message ||
        'Не вдалося отримати прогноз погоди',
    });
  }
};

export const getCurrentWeatherByCoords = async (req, res) => {
  try {
    ...
    if (!lat || !lon) {
      return res.status(400).json({
        success: false,
        message: 'Потрібні координати lat і lon',
      });
    }
    ...
    return res.status(error.response?.status || 500).json({
      ...
      message:
        error.response?.data?.message ||
        'Не вдалося отримати погоду за координатами',
    });
  }
};